{
    ...
    "medium": {
      ...
      "extraFieldsLeft": [
	  //Фон полоски ХП
      { "x": 250, "y": 6, "h": 16, "w": "{{hp-max:75}}", "src": "xvm://res/img/hp-bg.png","alpha": "{{alive?30|0}}"},
      //Полоска ХП
      { "x": 250, "y": 6, "h": 16, "w": "{{hp:75}}", "src": "xvm://res/img/hp-{{squad?squad|ally}}-{{alive}}.png","alpha": "{{alive?70|0}}"},
	  //Число ХП	
	  { "x": 250, "y": 5, "align": "left", "format": "<font size='12' color='{{c:hp-ratio}}'>{{hp| }}</font>", "alpha": "{{alive?100|0}}", "shadow": {"alpha": "{{alive?100|0}}", "size": 6, "distance": 0, "strength": 1, "color": "0x000000"} }
	  ],
      // Set of formats for right panel (extended format supported, see above)
      // Набор форматов для правой панели (поддерживается расширенный формат, см. выше)
      "extraFieldsRight": [
	  //Фон полоски ХП
      { "x": 255, "y": 6, "h": 16, "w": "{{hp-max:75}}", "src": "xvm://res/img/hp-bg.png","alpha": "{{alive?30|0}}"},
      //Полоска ХП
      { "x": 255, "y": 6, "h": 16, "w": "{{hp:75}}", "src": "xvm://res/img/hp-enemy-{{alive}}.png", "alpha": "{{alive?70|0}}"},
	  //Число ХП	
	  { "x": 255, "y": 5, "align": "right", "format": "<font size='12' color='{{c:hp-ratio}}'>{{hp| }}</font>", "alpha": "{{alive?100|0}}", "shadow": {"alpha": "{{alive?100|0}}", "size": 6, "distance": 0, "strength": 1, "color": "0x000000"} },
	  //Индикатор засвета
        ${"../Spotted/enemySpottedMarker.xc":"enemySpottedMarker"}
      ]
    },
    ...
    "large": {
      ...
      "extraFieldsLeft": [
	  //Фон полоски ХП
      { "x": 350, "y": 10, "h": 10, "w": "{{hp-max:75}}", "src": "xvm://res/img/hp-bg.png","alpha": "{{alive?30|0}}"},
      //Полоска ХП
      { "x": 350, "y": 10, "h": 10, "w": "{{hp:75}}", "src": "xvm://res/img/hp-{{squad?squad|ally}}-{{alive}}.png","alpha": "{{alive?70|0}}"},
	  //Число ХП	
	  { "x": 350, "y": 5, "align": "left", "format": "<font size='12' color='{{c:hp-ratio}}'>{{hp|?}}</font>", "alpha": "{{alive?100|0}}", "shadow": {"alpha": "{{alive?100|0}}", "size": 6, "distance": 0, "strength": 1, "color": "0x000000"} }
	  ],
      // Set of formats for right panel (extended format supported, see above)
      // Набор форматов для правой панели (поддерживается расширенный формат, см. выше)
      "extraFieldsRight": [
	  //Фон полоски ХП
      { "x": 345, "y": 10, "h": 10, "w": "{{hp-max:75}}", "src": "xvm://res/img/hp-bg.png","alpha": "{{alive?30|0}}"},
      //Полоска ХП
      { "x": 345, "y": 10, "h": 10, "w": "{{hp:75}}", "src": "xvm://res/img/hp-enemy-{{alive}}.png", "alpha": "{{alive?70|0}}"},
	  //Число ХП	
	  { "x": 345, "y": 5, "align": "right", "format": "<font size='12' color='{{c:hp-ratio}}'>{{hp| }}</font>", "alpha": "{{alive?100|0}}", "shadow": {"alpha": "{{alive?100|0}}", "size": 6, "distance": 0, "strength": 1, "color": "0x000000"} },
	  //Индикатор засвета
        ${"../Spotted/enemySpottedMarker.xc":"enemySpottedMarker"}
      ]
    }
